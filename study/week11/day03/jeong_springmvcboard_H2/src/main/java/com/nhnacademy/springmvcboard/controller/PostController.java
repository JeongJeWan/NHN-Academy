...
import com.nhnacademy.springmvcboard.Service.impl.DbPostServiceImpl;
...
public class PostController implements BaseController{
    ...
    public PostController(PostEventService postEventService) {
...
//        List<Post> postList = postService.getPosts();
        Page<Post> postPage = postEventService.getPostList(page, size);
...
//        model.addAttribute("postList", postList);
    ...
    public String register(@Valid PostRegisterRequest postRegisterRequest, BindingResult bindingResult, HttpServletRequest request) {
        ...
        long id = postEventService.register(post);
        post.setPostId(id);
        log.info("POST ID<><<><>><><:{}", id);
    ...
    public String update(@Valid PostRegisterRequest postRegisterRequest, BindingResult bindingResult, HttpServletRequest request) {
        ...
        postEventService.modify(postUpdate);
    ...
    public String view(Model model, @RequestParam(name = "postId", required = true) long postId) {
        Post post = postEventService.getPostById(postId);
        post.increasViewCount();
        postEventService.modify(post);
